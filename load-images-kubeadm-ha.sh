...
for i in "${linux_images[@]}"; do
    ...
    case $i in
        quay.io/coreos/*)
        image_name="${i/quay.io\/coreos/$reg/kubeadm-ha}"
        ;;
    quay.io/calico/*)
        image_name="${i/quay.io\/calico\//$reg/kubeadm-ha/calico_}"
        ;;
   k8s.gcr.io/metrics-server/*)
        image_name="${i/k8s.gcr.io\/metrics-server\//$reg/kubeadm-ha/metrics-server_}"
        ;;
  k8s.gcr.io/coredns/*)
       image_name="${i/k8s.gcr.io\/coredns/$reg/kubeadm-ha}"
       ;;
    k8s.gcr.io/*)
        image_name="${i/k8s.gcr.io/$reg/kubeadm-ha}"
        ;;

    docker.io/*)
        image_name="${i/docker.io/$reg/kubeadm-ha}"
        ;;

    *)
        echo "no math imag urli: $i"
        exit 2
        ;;
    esac
